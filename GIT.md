...
```sh
git clone https://git.unistra.fr/[votre login]/javascript-web-front.git
```
Vous vous retrouvez alors avec un dossier *javascript-web-front* qui contient une copie locale de votre dépôt.
...
```sh
git remote add upstream https://git.unistra.fr/joey.bronner/javascript-web-front.git
```
